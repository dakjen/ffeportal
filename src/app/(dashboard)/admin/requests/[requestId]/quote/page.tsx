  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Fetch current quote data to get its ID and status
        const quoteRes = await fetch(`/api/admin/quotes/${requestId}`);
        let currentQuoteId: string | undefined;
        let currentQuoteStatus: string | undefined;

        if (quoteRes.ok) {
          const quoteData = await quoteRes.json();
          currentQuoteId = quoteData.quote.id;
          currentQuoteStatus = quoteData.quote.status;
        }
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-gray-50 p-4 rounded-xl border border-dashed border-gray-300 min-h-[300px]">
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
              <SortableContext items={selectedQuoteItems.map(item => item.id!)} strategy={null}>
